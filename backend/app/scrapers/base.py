...
class Base:
    ...
    async def fetch_page(self, url: str, client: Optional[AsyncSession] = None) -> Optional[str]:
        ...
        try:
            ...
            if client:
                ...
                async with AsyncSession(impersonate="chrome124", headers=self.headers) as temp_client:
    ...
    async def fetch_soup(self, url: str, client: Optional[AsyncSession] = None) -> Optional[BeautifulSoup]:
        """ 
    ...
    def categorie_links(self, cat: Any) -> List[Dict[str, str]]:
        ...
        return [
            ...
            for i in cat if i.get("href") and i.get_text(strip=True)
        ]